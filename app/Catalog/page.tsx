...
const Vehicles = () => {
    ...
    const [editedVehicle, setEditedVehicle] = useState<vehicleInterface>();
    ...
    const renderCell = React.useCallback((user: vehicleInterface, columnKey: keyof vehicleInterface) => {
        ...
        switch (columnKey) {
            case "brand":
                return (
                    <User
                        avatarProps={{ radius: "lg", src: "https://www.shutterstock.com/image-vector/flat-car-picture-placeholder-symbol-600nw-2366856295.jpg", className: "mr-2" }}
                        description={user.model + " " + user.year}
                        name={user.brand}
                    >
                        {user.brand}
                    </User>
    ...
    }, [onOpen, setEditedVehicle]);

    return (
        <>
            <CreateClientModal isOpen={isOpen} onOpen={onOpen} onOpenChange={onOpenChange} refetch={refetch} editedVehicle={editedVehicle} setEditedVehicle={setEditedVehicle} />
                        ...
                        <TableBody>
                            {(filteredData || []).map((user) => (
                                <TableRow key={user.id}>
                                    {columns.map((column) => (
                                        <TableCell key={column.uid}>{renderCell(user, column.uid as any)}</TableCell>
                                    ))}
                                </TableRow>
                            ))}
                        </TableBody>
